...
(function (n, r, s) {
    ...
    TimeMap.prototype.functionName = function (fn) {
        ...
        return this.functionNameFromString(fn.toString());
    };


    /**
     * Return a function name from a string representation of the function.
     *
     ...
     */
    TimeMap.prototype.functionNameFromString = function (functionString) {
        var result;
        ...
        // Handle newlines and odd whitespace
        result = /^[\s\r\n]*function[\s\r\n]*([^\(\s\r\n]*?)[\s\r\n]*\([^\)]*\)[\s\r\n]*\{((?:[^}]*\}?)+)\}[\s\r\n]*$/.exec(functionString);
        /*jslint regexp:false*/
