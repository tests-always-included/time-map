...
(function (wind) {
	...
	function AngularProfiler(angular) {
		...
		this.profiler = new wind.FidProfile();
		angular.fidProfile = this.profiler;
		this.patchAngular();
		this.watcherHints = [];
	}
	...
	AngularProfiler.prototype.decorateRootScope = function ($provide) {
		...
		$provide.decorator('$rootScope', ['$delegate', function ($delegate) {
			...
			if (!$delegate.fidProfile) {
				$delegate.fidProfile = true;
	...
	AngularProfiler.prototype.delegation = function (originalFn, beforeFn, afterFn) {
		...
		returned = function () {
			...
			try {
				returned = originalFn.apply(this, args);

				if (afterFn) {
					returned = afterFn.call(this, returned, args);
				}
	...
	AngularProfiler.prototype.functionName = function (fn) {
		var name;
	...
	AngularProfiler.prototype.patchAngular = function () {
		...
		ngModule.config(['$provide', '$filterProvider', '$controllerProvider', '$compileProvider', function ($provide, $filterProvider, $controllerProvider, $compileProvider) {
	...
	AngularProfiler.prototype.patchScopeWatch = function (scope) {
		var myself;
		myself = this;
		scope.$watch = this.delegation(scope.$watch, function () {
	...
	// This must wrap only once, but that checking is handled by FidProfile
	...
	AngularProfiler.prototype.wrapInjector = function ($injector) {
		...
		// Only wrap once
		if ($injector.fidProfile) {
			return;
		}

		$injector.fidProfile = true;
		myself = this;

		// TODO:  Could also wrap invoke
		// TODO:  Could profile instantiate call
		// TODO:  Consider poking around with annotate to avoid
		...
		$injector.get = this.delegation($injector.get, null, function (returned, args) {
			...
			} else if (!returned.fidProfile) {
				// Only profile once as a speed booster
				returned.fidProfile = true;
	...
	AngularProfiler.prototype.wrapWatchers = function (scope) {
		...
		this.angular.forEach(scope.$$watchers, function (watcher) {
			...
			// This has the hint inside a hint, which makes it look crappy
			watcher.get = myself.profileInjectable(watcher.get, 'scope(' + scope.$id + ')', '$watch(' + hint + ').exp', exp);
